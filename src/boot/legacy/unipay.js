const wrapperUnitPay = () => {
  ...
  style.innerHTML =
    '.up-widget {z-index: 10000;height: 100%;width: 100%;position: fixed;left: 0;top: 0;' +
  ...
  var UnitPay = function (params) {
    ...
    this.createWidget = function (params) {
      ...
      let iframeFormUrl =
        'https\u003A//widget.unitpay.ru/pay/\u0040PUBLIC_ID\u0040'
      ...
      if (params['cashItems']) {
        if (
          Object.prototype.toString.call(params['cashItems']) ==
          '[object Array]'
        ) {
          params['cashItems'] = UnitPay.base64.encode(
            JSON.stringify(params['cashItems'])
          )
        }
      }

      if (
        params['paymentType'] === 'sms' ||
      ...
      if (isMobilePhone()) {
        var paymentFormUrl =
          'https\u003A//unitpay.ru/pay/\u0040PUBLIC_ID\u0040/\u0040BILLING_CODE\u0040'
        paymentFormUrl = paymentFormUrl.replace('@PUBLIC_ID@', publicKey)
        paymentFormUrl =
          paymentFormUrl.replace('@BILLING_CODE@', billingCode) + '?'
        params['hideMenu'] =
          typeof params['hideMenu'] == 'string' ? params['hideMenu'] : 'true'
  ...
  window.onmessage = function (event) {
    ...
    if (data.action == 'close') {
      ...
      var pr = document.body
        .getElementsByClassName('thumbnail-preview')
        .remove()
      setTimeout(function () {
        while (w[0]) {
